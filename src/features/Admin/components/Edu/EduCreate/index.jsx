import { useState } from 'react';
import queryString from 'query-string';
import { useLocation, useNavigate } from 'react-router-dom';
import { Snackbars } from '../../../../../components';

import educationApi from '../../../../../api/educationApi';
import { EduCreateForm } from '../EduCreateForm';
import { EduUpdateForm } from '../EduUpdateForm';

export function EduCreate() {
    const[message,setMessage] =useState()
    const[error,setError] =useState()
    const[isOpenSnackbar,setIsOpenSnackbar] = useState(false)
    const navigate = useNavigate();

    const location = useLocation();
    const params = queryString.parse(location.search).id;
    

    const handleSubmit =async (values)=>{
        try {
            let res;
            if(params){
                const queryParams={
                    id:params,
                    name:values.name,
                    status:values.status
                }
                res = await educationApi.updateEdu(queryParams);
            }else{
                const queryParams={
                    id:params,
                    name:values.name,
                }
                res = await educationApi.add(queryParams);
            }
            if(res.check ==false) {
                setIsOpenSnackbar(true)
                setError(res.msg.name[0])
            }
            else{
                setError(false)
                setIsOpenSnackbar(true)
                setMessage(res.msg)
                setTimeout(()=>{
                    navigate('/education');
                },2000)
            }
    
        } 
        catch (error) {
        console.log("Fail to fetch to product List", error);
        }

              
    }

    const CloseSnackbar = ()=>{
        setIsOpenSnackbar(false)
    }
    return (
        <div>
            {params ? (
                 <EduUpdateForm onSubmit={handleSubmit} ></EduUpdateForm>  
            ):(
                
                <EduCreateForm onSubmit={handleSubmit}></EduCreateForm>                   
            )}
                  
            { error ? (

                <Snackbars type='error' open={isOpenSnackbar} close={CloseSnackbar}
                anchor={{vertical: 'bottom', horizontal: 'right' }} content={error}></Snackbars>
            ) :
            (
                <Snackbars open={isOpenSnackbar} close={CloseSnackbar} type='success'
                anchor={{vertical: 'bottom', horizontal: 'right' }} content={message}></Snackbars>
            )
            }
        </div>
    );
}

